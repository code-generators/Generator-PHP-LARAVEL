...
{
    public class ApiControllersWritingSteps: StepBodyAsync
    {
        private readonly ISessionContext _context;
        private readonly IWriting _writingService;
        private readonly IWorkflowNotifier _workflowNotifier;

        public ApiControllersWritingSteps(ISessionContext context, IWriting writingService, IWorkflowNotifier workflowNotifier)
        {
            _context = context;
        ...
        public override Task<ExecutionResult> RunAsync(IStepExecutionContext context)
        {
            ...
            _workflowNotifier.Notify(nameof(ApiControllersWritingSteps), NotificationType.GeneralInfo, "Generating PHP apicontrollers");
        ...
        private void TransformApiControllers(SmartAppInfo smartApp)
        {
            if (smartApp != null && smartApp.Api != null)
            {
                ...
                foreach (var apiList in smartApp.Api)
                {
                    ...
                    _writingService.WriteFile(Path.Combine(_context.BasePath, template.OutputPath, (TextConverter.PascalCase(apiList.Id) + controllerSuffix + ".php")), template.TransformText());
                }
            }
        }
